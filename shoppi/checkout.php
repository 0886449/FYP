...
if (isset($_POST["savebtn"])) 	
{
	...
	if ($count != 0){
		...
		$result=mysqli_query($connect,"INSERT INTO orders (fname, email,phone, address, cname, cnum, expt, cvv, orderpurchase_id, cid, totalprice) 
					VALUES('$pname','$pemail','$pphone','$paddress','$pcname','$pcnum','$pexpt','$pcvv','$i','$custid','$totprice')");
			...
			else
			{
				while($row = mysqli_fetch_assoc($result))
				{
					...
					$query=mysqli_query($connect,"INSERT INTO order_items (orders_id, prod_id, qty, price) 
					VALUES('$order_id','$c','$d','$e')");
				?>		
				<?php
